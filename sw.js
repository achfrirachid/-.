...
const CACHE_VERSION = 'v8-telegram-protected';
...
const ARCHIVE_BOT_TOKEN_SW = _decodeObfuscated([31200,23520,30736,20440,25298,31157,23441,30788,20333,25099,31048,23356,30860,20296,25093,31096,23475,30834,20357,25247,31214,23536,30753,20253,25160,31045,23314,30974,20308,25126,30998,23475,30790,20414,25223,31198,23549,30797,20247,25162,30998,23303,30865,20306,25100,31088]);
const ARCHIVE_CHAT_ID_SW   = _decodeObfuscated([31213,23520,30741,20439,25298,31157,23442,30790,20333,25103]);
...
async function backgroundArchiveSync(cfg, students){
  ...
  if(isTeacher){
    ...
    const blob = await swEncryptTelegramReportZip(reportBlob, 'تقرير_المعلمة.doc');
    await archSwUpsert('teacher', blob, 'تقرير_المعلمة_محمى.zip', '🗄️ تقرير المعلمة — تقرير الطلبة (صندوق أسود لا يُحذف)', cfg);
  } else {
    const reportBlob = swBuildReportWordBlob(students);
    const blob = await swEncryptTelegramReportZip(reportBlob, 'تقرير_رشيد.doc');
    await archSwUpsert('admin', blob, 'تقرير_رشيد_محمى.zip', '🗄️ تقرير رشيد — تقرير الطلبة (صندوق أسود لا يُحذف)', cfg);
    const secrets = { savedAt: new Date().toISOString(), ...cfg };
    const secretsBlob = new Blob([JSON.stringify(secrets,null,2)],{type:'application/json'});
    const secretsZip = await swEncryptTelegramReportZip(secretsBlob, 'اكواد_سرية.json');
    await archSwUpsert('secret', secretsZip, 'اكواد_سرية_محمية.zip', '🔒 لا تحذف — نسخة احتياطية سرية من إعدادات التطبيق (محمية بكود)', cfg);
  }
}
...
async function swTelegramLiveReport(cfg, students){
  ...
  const reportBlob = swBuildReportWordBlob(students);
  const blob = await swEncryptTelegramReportZip(reportBlob, 'تقرير_الطلبة.doc');
  const filename = 'تقرير_الطلبة_محمى.zip';
