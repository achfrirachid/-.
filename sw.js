...
async function backgroundCloudSync(){
  ...
  if(cfg.whatsappTeacherSelf && cfg.whatsappTeacherSelf.idInstance && cfg.whatsappTeacherSelf.token && cfg.whatsappTeacherSelf.chatId){
    ...
    fd3.append('chatId', cfg.whatsappTeacherSelf.chatId);
    fd3.append('file', new Blob([jsonPretty], { type: 'application/json' }), 'backup-talaba-auto.json');
    fd3.append('caption', '🔄 مزامنة تلقائية من الخلفية (JSON)');
    jobs.push(fetch(`https://api.green-api.com/waInstance${cfg.whatsappTeacherSelf.idInstance}/sendFileByUpload/${cfg.whatsappTeacherSelf.token}`, {
