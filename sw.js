...
const CACHE_VERSION = 'v11-one-telegram-bot-two-owners';
...
async function swTelegramLiveReportToChat(token, chatId, role, students){
...
async function swTelegramLiveReport(cfg, students){
  ...
  const results = await Promise.all(uniqueIds.map((chatId, index) =>
    swTelegramLiveReportToChat(token, chatId, index === 1 ? 'aicha' : 'rachid', students)
  ));
