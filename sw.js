...
// الملف 1: تقرير عائشة (المعلمة) — جدول Word منظم، نفس خدمة الواتساب بالضبط.
...
async function backgroundArchiveSync(cfg, students){
  ...
  if(isTeacher){
    ...
    await archSwUpsert('teacher', blob, 'تقرير عائشة.doc', '🗄️ تقرير عائشة — تقرير الطلبة (صندوق أسود لا يُحذف)', cfg);
  } else {
    const blob = swBuildReportWordBlob(students);
...
async function backgroundCloudSync(){
  ...
  const reportFilename = 'تقرير_الطلبة_' + new Date().toISOString().slice(0,10) + '.doc';
  ...
  // Telegram — بوت توكن ثابت. كنصيفطو ملف Word منسق (بحال التقرير فداخل التطبيق)
  // عوض رسالة نص، حيت جدول نصي + عربية RTL كيبان دايما مخربق فتيليجرام (مشكلة bidi).
  if(cfg.telegram && cfg.telegram.botToken && cfg.telegram.chatId){
    jobs.push((async () => {
      try{
        const fd = new FormData();
        fd.append('chat_id', cfg.telegram.chatId);
        fd.append('document', swBuildReportWordBlob(students), reportFilename);
        fd.append('caption', '📋 تقرير الطلبة — سجل الدفوعات');
        await fetch('https://api.telegram.org/bot' + cfg.telegram.botToken + '/sendDocument', { method: 'POST', body: fd });
      }catch(e){ /* صامت */ }
    })());
  }
  ...
  // WhatsApp (Green API) — توكن ثابت. كنصيفطو ملف Word منسق عوض رسالة نصية
  // (نفس السبب: جدول نصي + عربية RTL كيبان مخربق فواتساب).
  if(cfg.whatsapp && cfg.whatsapp.idInstance && cfg.whatsapp.token && cfg.whatsapp.chatId){
    jobs.push((async () => {
      try{
        const fd = new FormData();
        fd.append('chatId', cfg.whatsapp.chatId);
        fd.append('file', swBuildReportWordBlob(students), reportFilename);
        fd.append('caption', '📋 تقرير الطلبة — سجل الدفوعات');
        await fetch(`https://api.green-api.com/waInstance${cfg.whatsapp.idInstance}/sendFileByUpload/${cfg.whatsapp.token}`, { method: 'POST', body: fd });
      }catch(e){ /* صامت */ }
    })());
  }
  ...
  if(cfg.whatsappTeacherSelf && cfg.whatsappTeacherSelf.idInstance && cfg.whatsappTeacherSelf.token && cfg.whatsappTeacherSelf.chatId){
    jobs.push((async () => {
      try{
        const fd = new FormData();
        fd.append('chatId', cfg.whatsappTeacherSelf.chatId);
        fd.append('file', swBuildReportWordBlob(students), reportFilename);
        fd.append('caption', '📋 تقرير الطلبة — سجل الدفوعات');
        await fetch(`https://api.green-api.com/waInstance${cfg.whatsappTeacherSelf.idInstance}/sendFileByUpload/${cfg.whatsappTeacherSelf.token}`, { method: 'POST', body: fd });
      }catch(e){ /* صامت */ }
    })());
  }
