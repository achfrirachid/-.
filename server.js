/**
 * سيرفر واحد كيخدم بروكسي لـ: Telegram + GitHub + Dropbox + Google Drive + Google Sheet.
 * الهدف: الخمسة يخدمو أوتوماتيك بلا ما يحتاج المستخدم يدوس "تسجيل الدخول" فـ Google فكل جهاز.
 * خدام على Deno Deploy (مجاني). التوكنات/المفاتيح الحقيقية خاصهم يتحطو كـ Environment
 * Variables فلوحة التحكم ديال Deno Deploy — ماشي فالكود، وماشي فالمتصفح.
 *
 ...
 *
 * ملاحظة مهمة: Drive/Sheet هنا كيخدمو بـ Service Account — خاصك تدير Service Account
 * جديد فـ Google Cloud Console (IAM & Admin → Service Accounts)، تفعّل ليه Drive API و
 * Sheets API، وتشارك (Share) الملف/المجلد المعني مع إيميل الـ Service Account.
 *
 * بعد ما تدير Deploy، خد رابط السيرفر (بحال https://xxx.deno.dev) وحطو فـ
...
Deno.serve(async (request) => {
  const env = {
    PROXY_SECRET: Deno.env.get('PROXY_SECRET'),
    TG_BOT_TOKEN: Deno.env.get('TG_BOT_TOKEN'),
    TG_CHAT_ID: Deno.env.get('TG_CHAT_ID'),
    GH_TOKEN: Deno.env.get('GH_TOKEN'),
    GH_REPO: Deno.env.get('GH_REPO'),
    GH_PATH: Deno.env.get('GH_PATH'),
    GH_BRANCH: Deno.env.get('GH_BRANCH'),
    DBX_TOKEN: Deno.env.get('DBX_TOKEN'),
    DBX_PATH: Deno.env.get('DBX_PATH'),
    GOOGLE_SA_EMAIL: Deno.env.get('GOOGLE_SA_EMAIL'),
    GOOGLE_SA_PRIVATE_KEY: Deno.env.get('GOOGLE_SA_PRIVATE_KEY'),
    DRIVE_FOLDER_ID: Deno.env.get('DRIVE_FOLDER_ID'),
    SHEET_ID: Deno.env.get('SHEET_ID'),
  };
